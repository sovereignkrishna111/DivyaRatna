...
import { usePeopleStore, Student } from '../hooks/usePeopleStore';
import Select from '../components/Select';
import { usePeopleTotals } from '../hooks/usePeopleTotals';

const Students: React.FC = () => {
  const { items, add, remove } = usePeopleStore<Student>('students');
  const { rows, upsert } = usePeopleTotals();
  const totals = rows.students;

  const [form, setForm] = React.useState<Omit<Student, 'id'>>({
  ...
  const saveCount = async () => {
    ...
    setSavingCount(true);
    try {
      await upsert('students', { total_count: next });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-maroon-800">Students Mode</h3>
            <p className="text-sm text-gray-600">Choose whether to add student details or only set the total count.</p>
          </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-maroon-800 mb-4">Add Student</h3>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-maroon-800">Students</h3>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {totals.mode !== 'count_only' && filtered.map((s) => (
              ...
              {totals.mode !== 'count_only' && filtered.length===0 && (
                <tr><td className="py-4 text-gray-500" colSpan={6}>No students yet.</td></tr>
              )}
              {totals.mode === 'count_only' && (
                <tr><td className="py-4 text-gray-500" colSpan={6}>Count Only mode is enabled. Individual student list is hidden.</td></tr>
              )}
            </tbody>
          </table>
        </div>
