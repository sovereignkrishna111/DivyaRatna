...
import { usePeopleStore, Staff } from '../hooks/usePeopleStore';
import { usePeopleTotals } from '../hooks/usePeopleTotals';

const StaffPage: React.FC = () => {
  const { items, add, remove } = usePeopleStore<Staff>('staff');
  const { rows, upsert } = usePeopleTotals();
  const totals = rows.staff;
  ...
  const saveCount = async () => {
    ...
    setSavingCount(true);
    try {
      await upsert('staff', { total_count: next });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-maroon-800">Staff Mode</h3>
            <p className="text-sm text-gray-600">Choose whether to add staff details or only set the total count.</p>
          </div>
        ...
        {totals.mode === 'count_only' && (
          <div className="mt-4 grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Total Staff (optional)</label>
              <input
                value={countDraft}
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-maroon-800 mb-4">Add Staff</h3>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-maroon-800 mb-4">Staff</h3>
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {totals.mode !== 'count_only' && items.map((t) => (
              ...
              {totals.mode !== 'count_only' && items.length===0 && (
                <tr><td className="py-4 text-gray-500" colSpan={4}>No staff yet.</td></tr>
              )}
              {totals.mode === 'count_only' && (
                <tr><td className="py-4 text-gray-500" colSpan={4}>Count Only mode is enabled. Individual staff list is hidden.</td></tr>
              )}
            </tbody>
          </table>
        </div>
